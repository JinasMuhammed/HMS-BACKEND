...
router.get('/', async (req, res) => {
    try {
        db.getAllDoc((err, doctors) => {
            if (err) return res.status(500).json({ error: "Error fetching doctors", details: err });

            db.getallappointment((err, appointments) => {
                if (err) return res.status(500).json({ error: "Error fetching appointments", details: err });
                ...
                console.log(' Sending API Response:', JSON.stringify(responseData, null, 2));
